...
createTricoreAsmTargetStreamer(MCStreamer &S, formatted_raw_ostream &OS,
...
class TricoreMCInstrAnalysis : public MCInstrAnalysis {
  TricoreMCInstrAnalysis(const TricoreMCInstrAnalysis &) = delete;
  TricoreMCInstrAnalysis &operator=(const TricoreMCInstrAnalysis &) = delete;
  virtual ~TricoreMCInstrAnalysis() = default;

public:
  TricoreMCInstrAnalysis(const MCInstrInfo *MCII) : MCInstrAnalysis(MCII) {}
...
bool TricoreMCInstrAnalysis::clearsSuperRegisters(const MCRegisterInfo &MRI,
                                                  const MCInst &Inst,
                                                  APInt &Mask) const {
  return false;
}
...
std::vector<std::pair<uint64_t, uint64_t>>
TricoreMCInstrAnalysis::findPltEntries(uint64_t PltSectionVA,
                                       ArrayRef<uint8_t> PltContents,
                                       const Triple &TargetTriple) const {
  return {};
}

bool TricoreMCInstrAnalysis::evaluateBranch(const MCInst &Inst, uint64_t Addr,
                                            uint64_t Size,
                                            uint64_t &Target) const {
  switch (TricoreII::getFormat(Info->get(Inst.getOpcode()).TSFlags)) {
  default:
    return false;
...
std::optional<uint64_t> TricoreMCInstrAnalysis::evaluateMemoryOperandAddress(
...
std::optional<uint64_t>
TricoreMCInstrAnalysis::getMemoryOperandRelocationOffset(const MCInst &Inst,
                                                         uint64_t Size) const {
  return std::nullopt;
}
} // namespace TricoreMC
...
static MCInstrAnalysis *createTricoreMCInstrAnalysis(const MCInstrInfo *Info) {
  return new TricoreMC::TricoreMCInstrAnalysis(Info);
}
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeTricoreTargetMC() {
  for (Target *T : {&getTheTricoreTarget()}) {
    ...
    TargetRegistry::RegisterMCInstrAnalysis(*T, createTricoreMCInstrAnalysis);
    ...
    TargetRegistry::RegisterAsmTargetStreamer(*T,
                                              createTricoreAsmTargetStreamer);

    // Register the null streamer.
    TargetRegistry::RegisterNullTargetStreamer(*T,
                                               createTricoreNullTargetStreamer);
